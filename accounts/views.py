    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = render_to_string('accounts/account_verification_email.html', {
                'user': user,
                'domain': current_site,
                'uid': urlsafe_base64_encode(force_bytes(user.pk)),#encoding userid by encoding the primary key while verfiying we will decode
                'token': default_token_generator.make_token(user),#generating token for the user
            })
            to_email = email
...
#now verifying the uidb and token generated while regisering
